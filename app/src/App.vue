...
document.title += getDateDiff((new Date()).getFullYear() + '-' + ((new Date()).getMonth() + 1) + '-' + (new Date()).getDate(), '2021-09-19') + 1 + '天';
export default {
    name: 'app',
    components: {
        StyleEditor,
    ...
    data() {
        return {
            ...
            fullStyle: [
                `/*
* Hi。宝贝！
* 在一起25天了，还没仔细给你看过我近期工作学习内容。
* 我嘛，以前就是个搬砖工程师傅，哪里需要轮子，哪里造。
* 自从我师傅离职后，我就开始了自己的研究提升。
* 此处省略一万万个字，先不感慨了。来看看花（专）里（业）胡（搬）哨（砖）：
* 比如这个页面。就是个什么也没有的网页。
* 我的工作就是给这种空白的页面加点儿东西。
* 嗯。说起来手机和电脑还得区分一下。
* 你现在用的是。。。${isPc ? '电脑' : '手机'}
